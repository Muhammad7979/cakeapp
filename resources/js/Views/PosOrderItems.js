...
const PosOrderItems = withAlert(({ history, location, alert }) => {
    ...
    const handleSelectRow = (e, row) => {
        ...
        if (selectedRow) {
            const totalPriceOfItem = parseFloat(selectedRow.unit_price) * parseFloat(selectedRow.receiving_quantity);

            if (checked) {
                // Update total state by adding the price of the selected item and calculated price
               
                    setTotal((prevTotal) => (parseFloat(prevTotal) + totalPriceOfItem ).toFixed(2));
                    setOriginalTotal((prevTotal) => (parseFloat(prevTotal) + totalPriceOfItem ).toFixed(2));
    ...
    const submitForm = async (e) => {
        ...
        if (cartItems.length <= 0) {
            ...
            try {
                ...
                console.log(data)
                let result = await axios.post('/onlyitems/save', data);
